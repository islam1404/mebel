...
const Card = ({ item }) => {
  const { favorites, favoritesHandler, user, addCarts , addCartsPlus } = useContext(CustomContext)
        ...
        <h4 className="card__content-price">{item.price}₽</h4>
        {
          ...
          ?
            <div className="card__sizes__count"
              ...
              <button style={{ padding: "10px 20px", background: "black", color: 'white', fontSize: "20px" }}>-</button>
            </div> :
